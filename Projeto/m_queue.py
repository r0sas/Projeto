...
def get_close(stock,time_dim):
    ...
    close_data = Queue(maxsize=time_dim)      #criação do queue onde iremos guardar os dados de close
    i = 0
    for row in rows:
        ...
        try:                                        #Irá verificar se a coluna não tem informações acerca de stocks_split
            ...
            close_data.put(float(cols[-2]))         #guardar os dados do close, que estão na última coluna
            i += 1;
        if i == time_dim:
            break
    return close_data
...
def get_rentability(close_data, time_dim):
    log_close_data = np.log(np.array(close_data.queue))
    rtbly = Queue(maxsize=time_dim-1)                    #onde irei guardar a rentabilidade
    for i in range(1,time_dim):
        rtbly.put(log_close_data[i] - log_close_data[i-1])
    rtn = sum(rtbly.queue) / (time_dim-1)                  #calculo do retorno
    return rtbly, rtn
...
def get_var_cv(rtbly, rtn, time_dim):
    var = 0
    for r in rtbly.queue:                                 #somatorio dos desvios ao quadrado
        var += (r - rtn)**2
...
def correlation(std_dev_i, std_dev_j, rtbly_i, rtbly_j, rtn_i, rtn_j, time_dim):
    ...
    for k in range(time_dim-1):
        cov_ij = cov_ij + (rtbly_i.queue[k] - rtn_i)*(rtbly_j.queue[k] - rtn_j)
...
for stock in tickers_lst:
    ...
    print("Standard Deviation: ", stocks_data[stock]["standard deviation"])
    plt.plot(x,np.log(stocks_data[stock]["close"].queue))
